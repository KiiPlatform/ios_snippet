...
private func snippet_1_non_blocking(){
  let userName = "user_123456"
  KiiUser.find(byUsername: userName) { (user , error ) -> Void in
    if error != nil {
      // Error handling
      return
    }
...
private func snippet_2_non_blocking(){
  let email = "user_123456@example.com"
  KiiUser.find(byEmail: email) { (user , error ) -> Void in
    if error != nil {
      // Error handling
      return
    }
...
private func snippet_3_non_blocking(){
  let phone = "+819012345678"
  KiiUser.find(byPhone: phone) { (user , error ) -> Void in
    if error != nil {
      // Error handling
      return
    }
...
private func snippet_4_non_blocking(){
  let userUri = "put existing user uri here"
  let userWithURI = KiiUser(uri: userUri)
  userWithURI.refresh { (user , error ) -> Void in
    if error != nil {
      // Error handling
      return
    }
...
private func snippet_5_non_blocking(){
  let userID = "__USER_ID_"
  let userWithID = KiiUser(uri: userID)
  userWithID.refresh { (user , error ) -> Void in
    if error != nil {
      // Error handling
      return
    }
