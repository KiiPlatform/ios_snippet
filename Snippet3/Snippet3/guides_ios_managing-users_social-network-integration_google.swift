...
private func snippet_1(){
  func myRegistrationMethod(){
    ...
    KiiSocialConnect.log(in: .Googleplus, options: options) { (retUser :KiiUser, provider : KiiConnectorProvider, retError : Error? ) -> Void in
      if (retError != nil) {
        // Error handling
        return
      }
...
private func snippet_4(){
  func myAccountLinkMethod(){
    ...
    KiiSocialConnect.linkCurrentUser(.Googleplus, options: options) { (retUser :KiiUser, provider : KiiConnectorProvider, retError : Error? ) -> Void in
      if (retError != nil) {
        // Error handling
        return
      }
...
private func snippet_5(){
  func myAccountUnlinkMethod(){
    ...
    KiiSocialConnect.unLinkCurrentUser(.Googleplus) { (retUser :KiiUser, provider : KiiConnectorProvider, retError : Error? ) -> Void in
      if (retError != nil) {
        // Error handling
        return
      }
