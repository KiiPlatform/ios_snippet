...
private func snippet_2_non_blocking(){
  KiiThing.load(withVendorThingID: "rBnvSPOXBDF9r29GJeGS") { (thing , error) -> Void in
    ...
    KiiUser.current()!.memberOfGroups({ (user , results : [AnyObject]?, error ) -> Void in
      if error != nil {
        // Error handling
        return
      }
...
private func snippet_4_non_blocking(){
  ...
  KiiThing.register(KiiUser.current()!, vendorThingID: vendorThingID) { (owner : KiiThingOwner, error ) -> Void in
    if error != nil {
      // Error handling
      return
    }
