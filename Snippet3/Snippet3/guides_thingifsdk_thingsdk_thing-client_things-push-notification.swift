...
private func snippet_1_non_blocking(){
  KiiThing.load(withVendorThingID: "rBnvSPOXBDF9r29GJeGS") { (thing , error) -> Void in
    ...
    let thingBucket = thing!.bucket(withName: "thing_bucket")
    KiiUser.current()!.pushSubscription().subscribe(thingBucket, block: { (subscription : KiiPushSubscription?, error ) -> Void in
      if error != nil {
        // Error handling
        return
      }
...
private func snippet_2_non_blocking(){
  KiiThing.load(withVendorThingID: "rBnvSPOXBDF9r29GJeGS") { (thing , error) -> Void in
    ...
    let thingBucket = thing!.bucket(withName: "thing_bucket")
    thing!.pushSubscription().subscribe(thingBucket, block: { (subscription : KiiPushSubscription?, error ) -> Void in
      if error != nil {
        // Error handling
        return
      }
...
private func snippet_3_non_blocking(){
  KiiThing.load(withVendorThingID: "rBnvSPOXBDF9r29GJeGS") { (thing , error) -> Void in
    ...
    let thingTopic = thing!.topic(withName: "thing_topic")
    KiiUser.current()!.pushSubscription().subscribe(thingTopic, block: { (subscription : KiiPushSubscription?, error ) -> Void in
      if error != nil {
        // Error handling
        return
      }
...
private func snippet_4_non_blocking(){
  KiiThing.load(withVendorThingID: "rBnvSPOXBDF9r29GJeGS") { (thing , error) -> Void in
    ...
    thingTopic.save({ (thingTopic , error ) -> Void in
      thing!.pushSubscription().subscribe(thingTopic, block: { (subscription : KiiPushSubscription?, error ) -> Void in
        if error != nil {
          // Error handling
          return
        }
