...
private func snippet_1a_blocking(){
  do{
    ...
    let group = try KiiGroup.registerSynchronous(withID: groupID, name: "myGroup", members: [KiiUser(id: "member's user_id")])
    // Get the reference URI.
    let groupUri = group.objectURI
...
private func snippet_1a_non_blocking(){
  ...
  KiiGroup.register(withID: groupID, name: "myGroup", members: [KiiUser(id: "member's user_id")]) { (group : KiiGroup?, error ) -> Void in
    if error != nil {
      // Group creation failed for some reasons.
      // Please check NSError to see what went wrong...
      return
...
private func snippet_2_non_blocking(){
  ...
  // (Assume that groupUri has the reference URI of the target group).
  let group2 = KiiGroup(uri: groupUri)
  group2.refresh{ (group2 : KiiGroup?, error ) -> Void in
    if error != nil {
      // Error handling
      return
    }
